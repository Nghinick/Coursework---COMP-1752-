...
def search_videos(search_term, search_by='name'):
    search_term = search_term.lower()
    results = []
    for key, item in library.items():
        if search_by == 'name' and search_term in item.name.lower():
            results.append((key, item))
        elif search_by == 'director' and search_term in item.director.lower():
            results.append((key, item))
    return results
